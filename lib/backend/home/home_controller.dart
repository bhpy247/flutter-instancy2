...
class HomeController {
  ...
  Future<void> getNewLearningResourcesListMain({bool isGetFromCache = false, required int componentId, required int componentInstanceId}) async {
    ...
    MyPrint.printOnConsole(
        "HomeController().getNewLearningResourcesListMain() called with isGetFromCache:$isGetFromCache, componentId:$componentId, "
  ...
  Future<void> getNewLearningResourcesList({bool isGetFromCache = false, required int componentId, required int componentInstanceId}) async {
    String tag = MyUtils.getNewId();
    MyPrint.printOnConsole(
        "HomeController().getNewLearningResourcesList() called with isGetFromCache:$isGetFromCache, componentId:$componentId, "
        "componentInstanceId:$componentInstanceId",
        tag: tag);
  ...
  Future<void> getPopularLearningResourcesListMain({bool isGetFromCache = false, required int componentId, required int componentInstanceId}) async {
    String tag = MyUtils.getNewId();
    MyPrint.printOnConsole(
        "HomeController().getPopularLearningResourcesListMain() called with isGetFromCache:$isGetFromCache, componentId:$componentId, "
        "componentInstanceId:$componentInstanceId",
        tag: tag);
  ...
  Future<void> getPopularLearningResourcesList({bool isGetFromCache = false, required int componentId, required int componentInstanceId}) async {
    String tag = MyUtils.getNewId();
    MyPrint.printOnConsole(
        "HomeController().getPopularLearningResourcesList() called with isGetFromCache:$isGetFromCache, componentId:$componentId, "
        "componentInstanceId:$componentInstanceId",
        tag: tag);
  ...
  Future<void> getRecommendedCourseListMain({bool isGetFromCache = false, required int componentId, required int componentInstanceId}) async {
    String tag = MyUtils.getNewId();
    MyPrint.printOnConsole(
        "HomeController().getRecommendedCourseListMain() called with isGetFromCache:$isGetFromCache, componentId:$componentId, "
        "componentInstanceId:$componentInstanceId",
        tag: tag);
  ...
  Future<void> getRecommendedCourseList({bool isGetFromCache = false, required int componentId, required int componentInstanceId}) async {
    String tag = MyUtils.getNewId();
    MyPrint.printOnConsole(
        "HomeController().getRecommendedCourseList() called with isGetFromCache:$isGetFromCache, componentId:$componentId, "
        "componentInstanceId:$componentInstanceId",
        tag: tag);
    BuildContext context = NavigationController.mainNavigatorKey.currentContext!;
    AppSystemConfigurationModel appSystemConfigurationModel = context.read<AppProvider>().appSystemConfigurationModel;

    bool isRecommendedContentEnabled = appSystemConfigurationModel.isContentRecommendationEnabled;
    MyPrint.printOnConsole("isRecommendedContentEnabled : $isRecommendedContentEnabled");

    DataResponseModel<CategoryWiseCourseDTOResponseModel> response = !isRecommendedContentEnabled
        ? await homeRepository.getNewResourceList(
            componentId: componentId,
            componentInstanceId: componentInstanceId,
            isStoreDataInHive: !isGetFromCache,
            isFromOffline: isGetFromCache,
          )
        : await homeRepository.getRecommendedCourseList(
            componentId: componentId,
            componentInstanceId: componentInstanceId,
            isStoreDataInHive: !isGetFromCache,
            isFromOffline: isGetFromCache,
